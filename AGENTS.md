...
- Backend: `copy example.env .env`. Frontend: `copy .env.example .env`. Keys: backend `PORT,NODE_ENV,DB_URL,FR_ORIGIN,CLOUDINARY_URL,CLERK_PUBLISHABLE_KEY,CLERK_SECRET_KEY,REDIS_URL`; frontend only `VITE_API_URL,VITE_CLERK_PUBLISHABLE_KEY`.
- Frontend + backend Clerk keys must come from the same Clerk app. Missing frontend key = admin shows "disabled", public pages still work.
...
- Frontend calls: use `apiFetch(path, opts)` from `frontend/src/api/client.js` — base is `VITE_API_URL`, always `credentials: 'include'`, auto-injects `Authorization: Bearer <Clerk JWT>`. Throws with `error.status`/`error.body` on non-OK.
...
- Frontend: `index.html → src/main.jsx` (Clerk gate) `→ src/App.jsx` (`BrowserRouter`). Public routes `/, /about, /team, /events[/:slug], /gallery[/:slug], /partners, /contact`; admin `/admin/login/*` open, everything else under `<ProtectedRoute><AdminShell>`. Keep both `vercel.json` SPA rewrites (`/(.*) → /index.html`) or deep links refresh to 404.
