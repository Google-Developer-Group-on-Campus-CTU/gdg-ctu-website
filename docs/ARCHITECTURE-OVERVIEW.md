...
**One-sentence version:** the React website (frontend) shows pages to visitors and asks a Node server (backend) for data; the server checks logins with Better Auth (an HTTP-only session cookie), reads/writes the database, and stores photos in Cloudinary.
...
flowchart LR
    ...
    Frontend -->|apiFetch with session cookie| Backend["Backend (Express) at /GDGoC-CTU-Main/v0.0.1"]
    Backend -->|check login| BetterAuth["Better Auth (session cookie check)"]
...
In words: (1) the visitor opens a page, (2) the frontend calls the backend with `apiFetch` (always `credentials: 'include'`, so the Better Auth session cookie is sent if signed in), (3) the backend verifies that session with Better Auth, (4) it reads the database and returns the answer, (5) photos go to/from Cloudinary. Public pages skip the login step; `/admin` pages require it (`ProtectedRoute` in `App.jsx` sends strangers to `/admin/login`).

## Folder map (where to look)

- `backend/src/server.ts` — the starting point: loads settings, allows only the `FR_ORIGIN` website address list (CORS), mounts the Better Auth login handler, and mounts all features at `/GDGoC-CTU-Main/v0.0.1`.
...
- `frontend/src/main.jsx` — starts the website (no auth provider needed — Better Auth talks straight to the backend, see `src/lib/auth-client.ts`); `frontend/src/App.jsx` — all page addresses.
- `frontend/src/api/client.js` — the only file that talks to the backend (`apiFetch` sends the session cookie via `credentials: 'include'` and applies the `VITE_API_URL` prefix).
...
2. **Auth (401 vs 403)** — Better Auth is the login system (email/password + optional Google; the browser keeps only an HTTP-only session cookie, the backend stores the rest). `401` means "we don't know who you are — please log in." `403` means "we know you, but your account isn't an active admin." If you see these, it's a login/permission issue, not broken code.
3. **Versioned base path + CORS + `VITE_API_URL`** — every backend address starts with `/GDGoC-CTU-Main/v0.0.1` (the version, so future changes don't break today's site). CORS is a browser safety rule: the backend answers only the website address(es) listed in `FR_ORIGIN` (a comma-separated allowlist; trailing `/` are trimmed, and development also allows any `localhost`/`127.0.0.1` loopback port). `VITE_API_URL` is the frontend's copy of the backend address — both must match or calls fail.
...
Base path for everything: `/GDGoC-CTU-Main/v0.0.1` (so the health check is `GET <backend>/health` — the same liveness JSON also answers `GET <backend>/` and `GET <backend>/GDGoC-CTU-Main/v0.0.1/health`).

| Route family | Example | Who can use it |
|---|---|---|
| Liveness | `GET /`, `GET /health`, `GET /GDGoC-CTU-Main/v0.0.1/health` | Anyone (deployment health probes) |
| Public reads | `GET /public/events`, public team/gallery/partner/content listings | Anyone, no login |
| Admin writes | `POST / PATCH / DELETE` on `/admins`, events, team, partners, gallery, content, media, settings | Signed-in active admin only (Better Auth session required; otherwise `401`/`403`) |
